...
<body>
    <div class="flip-card" id="card-{flip_gen}" onclick="
...
<script>
// เริ่มการ์ดใหม่ทุกครั้งที่ generation เปลี่ยน (คำใหม่) ให้แสดง "หน้า" (คำจีน)
// เสมอ ไม่ใช้ค่า flipped ที่อาจค้างมาจาก component instance ก่อนหน้า
...
""", height=400, scrolling=False, key=f"flip_card_component_{flip_gen}")
            ...
            #
            # หมายเหตุ: ตอนนี้ให้ key ของ components.html เป็น "ต่อ generation"
            # (key=f"flip_card_component_{flip_gen}") ทำให้ทุกครั้งที่เปลี่ยนคำ
            # (generation ใหม่) เป็น component instance ใหม่ล้วนๆ ค่าที่เคยค้าง
            # จาก instance เก่า (เช่น flipped=True) จะไม่ถูกนำมาปนกับคำใหม่อีก
            # ต่อไป — แก้ปัญหาการ์ดเริ่มที่ "หน้าหลัง" (เฉลย) แทนที่จะเป็น
            # "หน้าคำจีน" ตอนขึ้นคำใหม่
