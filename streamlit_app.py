...
def _save_progress():
    """บันทึกความคืบหน้า — เขียนเฉพาะ "แถวของตัวเอง" ลง Google Sheet
    (key "history" ที่เป็นกลาง + key "players:<ชื่อฉัน>" ที่เป็นของฉันคนเดียว)
    เพื่อไม่ให้เขียนทับ SRS/สถิติของผู้เล่นคนอื่นที่อาจกำลังเล่นพร้อมกันอยู่"""
    ...
    # ตัด history ให้เหลือแค่ล่าสุด MAX_HISTORY แถว กัน cell บน Google Sheet
    # โตเกิน 50,000 ตัวอักษรตอนเล่นไปนานๆ (ตัดทั้งใน session ด้วยเลย ไม่ใช่
    # แค่ตอนเซฟ เพื่อไม่ให้ list ใน session โตไม่มีที่สิ้นสุด)
    trimmed_history = st.session_state.get("play_history", [])[-MAX_HISTORY:]
    st.session_state.play_history = trimmed_history

    ws = _get_gsheet_worksheet()
    if ws is not None:
        try:
            cache = _gsheet_read_all(ws)
            _gsheet_upsert_row(ws, "history", trimmed_history, cache)
    ...
    # เก็บทุกคนไว้ในไฟล์เดียวได้ตามปกติ เพราะเป็น local file ไม่มีปัญหาแย่งเขียน
    # ข้ามคนแบบ Google Sheet (แต่ละ session ก็รันบน process เดียวกันอยู่แล้ว)
    try:
        local_data = {
            "srs": srs_all,
